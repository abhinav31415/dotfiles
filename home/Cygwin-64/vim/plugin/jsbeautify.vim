...
function! g:Jsbeautify()
    ...
    let s:line_starters = split("continue,try,throw,return,var,if,switch,case,default,for,while,break,else if,foreach", ",")

    let s:block_enders = split("else,catch,finally", ",")
		...
		try
			...
			elseif s:token_type == "TK_START_BLOCK"
				...
				if s:last_type != "TK_OPERATOR" && s:last_type != "TK_START_EXPR"
					if s:last_type == "TK_START_BLOCK" || s:last_type == "TK_END_EXPR" || s:in_array(s:last_word, s:block_enders)
						call s:print_newline(1)
					else
						call s:print_space()
					endif
